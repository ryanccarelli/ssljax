...
class MultiCrop(Model):
    ...
    resolution are clubbed. Outputs returned concatenated. Note that we do not
    apply the head.
    ...
    def forward(self, x):
        ...
        start_idx, output = 0, jnp.empty(0)
        for end_idx in idx_crops:
            _out = self.module(jnp.concatenate(x[start_idx: end_idx]))
