...
from ssljax.augment import Augmentation
...
class RandomFlip(Augmentation):
    ...
    def __call__(self, x, rng):
        ...
        return jax.vmap(self._random_flip_single_image)(x, rngs)
...
class RandomGaussianBlur(Augmentation):
    def __call__(self, x, rng, kernel_size, padding, sigma_min, sigma_max, apply_prob):
        ...
        return self._maybe_apply(_apply, x, apply_rng, apply_prob)
...
class Solarize(Augmentation):
    ...
    def __call__(self, x, rng, threshold=0.5, apply_prob=1.0):
        rngs = jax.random.split(rng, x.shape[0])
