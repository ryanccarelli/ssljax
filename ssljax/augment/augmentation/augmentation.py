...
class RandomCrop(Augmentation):
    ...
    def __call__(self, x, rng):
        ...
        random_crop_fn = functools.partial(
            self._decode_random_crop_single_image, crop_height=self.height, crop_width=self.width
        )
        return jax.vmap(random_crop_fn)(x, rngs)

    def _random_crop(self, img: np.ndarray, crop_height: int, crop_width: int, h_start: float = 0, w_start: float = 0):
        ...
        x1, y1, x2, y2 = self._get_random_crop_coords(height, width, crop_height, crop_width, h_start, w_start)
        img = img[y1:y2, x1:x2]
        return img

    def _get_random_crop_coords(self, height: int, width: int, crop_height: int, crop_width: int, h_start: float, w_start: float):
...
class CenterCrop(Augmentation):
    ...
    def _get_center_crop_coords(self, height: int, width: int, crop_height: int, crop_width: int):
    ...
    def _center_crop(self, img: np.ndarray, crop_height: int, crop_width: int):
        height, width = img.shape[:2]
        if height < crop_height or width < crop_width:
            raise ValueError(
                "Requested crop size ({crop_height}, {crop_width}) is "
        ...
        x1, y1, x2, y2 = self._get_center_crop_coords(height, width, crop_height, crop_width)
        img = img[y1:y2, x1:x2]
        return img
