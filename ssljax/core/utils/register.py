...
def register(type_, name,override=False):
  ...
  Args:
  type_ (class/callable): Specify the type of the class/module being registered.
  ...
  def register_entity(entity):
    ...
    if not callable(entity):
      raise ValueError(
                "Entity  doesn't match input type nor is a callable ({}: {})".format(name, entity.__name__)
            )
