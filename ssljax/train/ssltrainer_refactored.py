...
def train(
    *,
    ...
    # training steps
    steps_per_epoch = (
        task.data.meta_data.get("num_train_examples", 0) // task.config.env.batch_size
    )
    total_steps = steps_per_epoch * task.config.env.epochs

    # TODO: pmap training step here
    train_step_pmapped = None
    # TODO: pmap eval step here
    eval_step_pmapped = None
    ...
    total_eval_steps = int(
        np.ceil(task.data.meta_data["num_eval_examples"] / task.config.env.batch_size)
    )
    ...
    chrono = train_utils.Chrono(
        ...
        global_bs=task.config.env.batch_size,
    ...
    steps_per_epoch = (
        ...
        // task.config.data.params.batch_size
    )
