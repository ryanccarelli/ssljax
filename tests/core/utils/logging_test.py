...
def test_reset_tqdm_logger_handlers(tmpdir):
    ...
    common_logging.prepare_global_logging(
        serialization_dir_a,
        log_file_name="test_reset_tqdm_logger_handlers"
    )
    ...
    with open(os.path.join(serialization_dir_a, "test_reset_tqdm_logger_handlers.log"), "r") as f:
        assert len(f.readlines()) == 0
    with open(os.path.join(serialization_dir_b, "test_reset_tqdm_logger_handlers.log"), "r") as f:
